...
_Last updated: 2026-08-30 (daily scheduled check). This file is meant to be updated by the daily scheduled Claude Code task after checking recent commits, and read by Dallas as the source of truth for what's done vs. pending._

## ⚠️ Process problem (unresolved for 6+ weeks — read this first)
`main` has had zero commits since 2026-07-20 (last real code commit 07-15) — 41/46 days now. Every daily check since 07-21 has pushed its findings to a new throwaway `claude/gifted-hopper-*` branch instead of `main`; **34 of these branches now exist, and zero have ever been opened as a PR or merged.** This session is scoped to develop on `claude/gifted-hopper-y2j9ho` specifically and is not authorized to push to `main` or merge other branches on its own — that needs Dallas to either merge a branch himself or update the scheduled task's instructions. Until then, expect this section to keep re-appearing. Dallas's own real commit, `gitignore-security-fixes` (2026-08-05, adds `.gitignore` exclusions for `.sql`/`.env`/credential files), is also still sitting unmerged (25 days).
**Recommendation: merge the most recent tracker branch (`claude/gifted-hopper-lp3c4j` or this run's `claude/gifted-hopper-y2j9ho`) and `gitignore-security-fixes` into `main`, or explicitly tell the scheduled task to push straight to `main` going forward.**

No new commits landed on `card-scout` in the last 24 hours (checked all branches, 2026-08-29 13:14 UTC → now). Code is unchanged from yesterday's check, so today's findings are a re-verification, not new discoveries — the three previously-flagged items (security hole, dashboard bug, cost-basis bug) were spot-checked again directly in code and are all still present, unchanged. Also confirmed today: no test files/directories and no `.github` CI workflows exist anywhere in the repo.
...
- [ ] No test files or test directories exist anywhere in the repo — confirmed today. Given the repo is about to take on cost-basis integrity, sold-item sync, and eBay OAuth (all money-affecting logic), even minimal unit tests around cost-basis calculation and sold-item locking would be worth more than average right now, before those land.
...
- [ ] No lint/CI gate exists to catch bugs like the `tickFormatter` duplicate-prop above — `eslint-plugin-react`'s `react/jsx-no-duplicate-props` rule would flag that exact class of bug at PR time for free. Worth pairing with the "No CI workflow" backlog item above: a minimal GitHub Actions job running typecheck + lint on push would have caught this one automatically before it shipped.
- [ ] Given the branch-sprawl problem above, have this scheduled task open/update one persistent PR for its tracker updates (or push straight to `main`) instead of creating a brand-new branch every run — 34 dangling branches with zero PRs after 6 weeks is itself worth fixing at the source, not just noting.
- [ ] Investor-Dashboard reportedly already has a working eBay OAuth flow that Card Scout needs to port (see "In Progress" above), and Card Scout already has auth-check logic duplicated three ways internally (see item above). If the two apps are expected to eventually merge anyway (see P/L tab item above), it may be worth extracting eBay auth + the shared-middleware auth check into one small shared package now rather than porting logic twice and then de-duplicating it again later.
