...
_Last updated: 2026-09-04 (daily scheduled check). This file is meant to be updated by the daily scheduled Claude Code task after checking recent commits, and read by Dallas as the source of truth for what's done vs. pending._

## ⚠️ Process problem (unresolved for 7+ weeks — read this first)
`main` has had zero commits since 2026-07-20 (last real code commit 07-15) — 46/51 days now. Every daily check since 07-21 has pushed its findings to a new throwaway `claude/gifted-hopper-*` branch instead of `main`; **38+ of these branches now exist, and zero have ever been opened as a PR or merged.** This session is scoped to develop on `claude/gifted-hopper-ytj2ak` specifically and is not authorized to push to `main` or merge other branches on its own — that needs Dallas to either merge a branch himself or update the scheduled task's instructions. Until then, expect this section to keep re-appearing. Dallas's own real commit, `gitignore-security-fixes` (2026-08-05, adds `.gitignore` exclusions for `.sql`/`.env`/credential files), is also still sitting unmerged (30 days).

**Recommendation: merge the most recent tracker branch (this run's `claude/gifted-hopper-ytj2ak`) and `gitignore-security-fixes` into `main`, or explicitly tell the scheduled task to push straight to `main` going forward.**

No new commits landed on `card-scout` in the last 24 hours (checked all branches, 2026-09-03 13:25 UTC → now — the only activity in that window was yesterday's own tracker-update commit on `claude/gifted-hopper-fn91hz`). Code is unchanged, so today's findings are a re-verification carried forward from 09-03, not new discoveries. Re-confirmed directly against source: the `storage.ts` auth check is still commented out, the `collection.ts` cost-basis overwrite still has no guard, and the `Dashboard.tsx` `tickFormatter` duplicate prop is still present (all unchanged, see below).

## 🔴 Security — fix now
- `GET /storage/objects/*path` (`artifacts/api-server/src/routes/storage.ts`, lines 139-149) serves private object-storage files with **zero auth or ACL check** — the check is fully written but commented out ("Protected route example — uncomment when using replit-auth"). Anyone who knows or guesses an object path can fetch it. The auth check and `canAccessObjectEntity` helper already exist in the file — this is a ~10-line uncomment-and-wire fix, not new work. Known and unfixed since 2026-08-24 (11 days).
...
- **eBay CSV import — NOT built.** No CSV import route exists anywhere in `artifacts/api-server/src/routes`, only CSV *export* on the Collection/Insurance pages. This kept getting marked done in error whenever a run based its update on stale `main` instead of the latest accurate branch — flagging here again since `main` itself still shows the old, incorrect version of this file.
...
- [ ] Confirm whether the "In Progress / Just Sent to Replit" items are actually being worked in Replit's private workspace and just not syncing to GitHub — 51 days with zero commits despite four items marked "in progress" is long enough that it's worth checking whether the Replit↔GitHub push integration is still connected, rather than assuming the work simply hasn't started.
- [ ] Given 46 straight days of "no new commits," consider dropping this scheduled check from daily to weekly (or pausing it) until active development resumes — it currently burns a full run every day to re-confirm the same unchanged state.
- [ ] Once a branch is merged, clean up the 38 orphaned `claude/gifted-hopper-*` branches — none of them carry information that isn't now consolidated in this file, and they make the branch list unusable for finding real work.
- [ ] Add a one-line CI/pre-commit guard (a grep step is enough) that fails the build if `storage.ts`'s auth block on the object-storage route stays commented out. The fix has been "known and ready" for 11 days without landing; a hard gate is cheaper than another day of this file re-flagging it.
- [ ] Stop the branch bleeding at the source instead of relying on a future cleanup pass: point the scheduled tracker task at one stable branch name (e.g. reuse `gitignore-security-fixes`, or create `progress-tracker` once) and have it force-push there each run, rather than spawning a fresh `claude/gifted-hopper-*` branch daily.
- [ ] Right now every daily check is half-blind — this session's GitHub scope covers `card-scout` only, so the "Investor-Dashboard already has working eBay OAuth" claim above has never actually been verified against that repo's code, just repeated from earlier notes. Worth extending this scheduled task's access to `Investor-Dashboard` (read-only is enough) so future checks can confirm that OAuth flow directly instead of taking it on faith.
- [ ] **(New 09-04)** `PUT /collection/:id`'s fee-field updates (`purchasePrice`, `gradingFee`, `shippingFee`, `otherFees` — `routes/collection.ts` lines 146-153) are cast with bare `String(body.x)` and never validated, unlike `POST /cards`'s sibling routes. A negative number, non-numeric string, or `NaN` written here silently corrupts `trueCost`/gain math downstream (line 215). Since this file is about to become money-critical (cost-basis integrity item above), this route needs the same zod treatment as the `POST /cards` fix, not just that one.
- [ ] **(New 09-04)** `routes/collection.ts` already computes per-item `gain` (`currentValue - purchasePrice`, line 34) for the summary endpoint — a cheap "Top gainers / losers" widget on the Dashboard could reuse that exact calculation with no new backend work, just a sort + slice and a small UI card.
- [ ] **(New 09-04)** Cards have no visible "price last refreshed" timestamp even though `price_history` already timestamps every eBay price pull. Given the eBay Browse API can silently fail for a given item (see the retry/backoff gap above), a per-card "last synced Xh ago" badge would make stale data visible to Dallas instead of failing silently — pairs naturally with the Sync Now / sold-item sync work already planned.
