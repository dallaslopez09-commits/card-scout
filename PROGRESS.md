...
_Last updated: 2026-09-02 (daily scheduled check). This file is meant to be updated by the daily scheduled Claude Code task after checking recent commits, and read by Dallas as the source of truth for what's done vs. pending._

## ⚠️ Process problem (unresolved for 6+ weeks — read this first)
`main` has had zero commits since 2026-07-20 (last real code commit 07-15) — 44/49 days now. Every daily check since 07-21 has pushed its findings to a new throwaway `claude/gifted-hopper-*` branch instead of `main`; **36+ of these branches now exist, and zero have ever been opened as a PR or merged.** This session is scoped to develop on `claude/gifted-hopper-7u50dp` specifically and is not authorized to push to `main` or merge other branches on its own — that needs Dallas to either merge a branch himself or update the scheduled task's instructions. Until then, expect this section to keep re-appearing. Dallas's own real commit, `gitignore-security-fixes` (2026-08-05, adds `.gitignore` exclusions for `.sql`/`.env`/credential files), is also still sitting unmerged (28 days).

Also worth flagging: at least one prior daily run (2026-09-01) built its update from stale `main` instead of the most recent accurate branch, and silently regressed several corrections (re-marked "eBay CSV import built" as done, dropped the storage.ts security finding). This run rebuilds from the 2026-08-31 branch (the last fully-verified state) instead of 09-01's regressed one, to avoid losing those corrections again.

**Recommendation: merge the most recent tracker branch (this run's `claude/gifted-hopper-7u50dp`) and `gitignore-security-fixes` into `main`, or explicitly tell the scheduled task to push straight to `main` going forward.**

No new commits landed on `card-scout` in the last 24 hours (checked all branches, 2026-09-01 13:27 UTC → now). Code is unchanged, so today's findings are a re-verification carried forward from 08-31, not new discoveries.

## 🔴 Security — fix now
- `GET /storage/objects/*path` (`artifacts/api-server/src/routes/storage.ts`) serves private object-storage files with **zero auth or ACL check** — the check is fully written but commented out ("Protected route example — uncomment when using replit-auth", lines 139-149). Anyone who knows or guesses an object path can fetch it. The auth check and `canAccessObjectEntity` helper already exist in the file — this is a ~10-line uncomment-and-wire fix, not new work. Known and unfixed since 2026-08-24 (9 days).
...
- [ ] **(New)** Confirm whether the "In Progress / Just Sent to Replit" items are actually being worked in Replit's private workspace and just not syncing to GitHub — 49 days with zero commits despite four items marked "in progress" is long enough that it's worth checking whether the Replit↔GitHub push integration is still connected, rather than assuming the work simply hasn't started.
- [ ] **(New)** Given 44 straight days of "no new commits," consider dropping this scheduled check from daily to weekly (or pausing it) until active development resumes — it currently burns a full run every day to re-confirm the same unchanged state.
- [ ] **(New)** Once a branch is merged, clean up the 36 orphaned `claude/gifted-hopper-*` branches — none of them carry information that isn't now consolidated in this file, and they make the branch list unusable for finding real work.
